...
function getNextPrecision(previous_result) {
  const result = {
    ...previous_result,
  }
  ...
  const nextPiPrecision =
    ((result.i * 27n - 12n) * result.q + result.r * 5n) / (result.t * 5n)
  // update result needed for next iteration of getNextPrecision
  let u = result.i * 3n
  u = (u + 1n) * 3n * (u + 2n)
  ...
  result.r =
    u *
    10n *
    (result.q * (result.i * 5n - 2n) + result.r - result.t * nextPiPrecision)
  result.q *= 10n * result.i * (result.i++ * 2n - 1n)
  result.t *= u
  result.pi = bigInt(result.pi)
    .toString()
