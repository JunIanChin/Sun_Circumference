...
const resetPiHandler = require('../services/resetPi')
...
describe('Function Test, get next Pi precision value', function () {
  it('Function get_nextPiPrecision should return correct transformedData for next pi preicison', async function () {
    ...
    const nextPiPrecision =
      ((transformedData.i * 27n - 12n) * transformedData.q +
        transformedData.r * 5n) /
      (transformedData.t * 5n)
    ...
    transformedData.r =
      u *
      10n *
      (transformedData.q * (transformedData.i * 5n - 2n) +
        transformedData.r -
        transformedData.t * nextPiPrecision)
    transformedData.q *=
      10n * transformedData.i * (transformedData.i++ * 2n - 1n)
    transformedData.t *= u
    transformedData.pi = bigInt(transformedData.pi)
      .toString()
      .concat(bigInt(nextPiPrecision).toString())

    expect(parseInt(updatedPiValue.q, 10)).to.equal(
      bigInt(transformedData.q).toJSNumber(),
    )
    expect(parseInt(updatedPiValue.r, 10)).to.equal(
      bigInt(transformedData.r).toJSNumber(),
    )
    expect(parseInt(updatedPiValue.t, 10)).to.equal(
      bigInt(transformedData.t).toJSNumber(),
    )
    expect(parseInt(updatedPiValue.i, 10)).to.equal(
      bigInt(transformedData.i).toJSNumber(),
    )
    expect(parseInt(updatedPiValue.pi, 10)).to.equal(
      bigInt(transformedData.pi).toJSNumber(),
    )
  })
})
